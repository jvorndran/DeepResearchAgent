...
2. **Fetch:** `fred_get_series` for each series. If the result is `status:auto_saved`, keep the returned `file_path` and do not call `save_data`; call `save_data` only for successful results that were not already saved.
...
5. **Paths:** Use absolute paths consistently for all tools (`execute`, `pandas.read_csv`, and filesystem tools).
6. **Quarter Labels:** Format quarterly labels as `YYYY Qn`; never use unsupported `strftime` directives like `%Q`.
7. **Charts:** 
