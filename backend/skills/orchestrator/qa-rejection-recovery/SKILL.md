...
1. **Rule of Three:** Max 3 structured QA rejections/report-gate failures total. If the 3rd repair still fails or gets rejected, abort with a concise QA-rejected status.
2. **Analysis:** Read `reason` and `required_fixes` from the latest `quality-analyst` JSON result. Do not re-run QA just to recover details and do not inspect artifacts yourself.
3. **Re-delegate:**
   ...
   - **Quant-owned fixes:** Send back to `quant-developer` when QA says computed artifacts are missing, stale, invalid, need recalculation/new analysis, a requested analog/window/backtest row is missing from `execution_summary.json`, a requested chart family or chart definition is missing from `charts.json`, or the report has zero chart definitions / missing `charts.json` for a chart-required request. Do not ask quant to patch narrative wording or inspect `report.json`.
